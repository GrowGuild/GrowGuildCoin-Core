...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("growguildcoin"))
        return false;
...
{
    // Convert growguildcoin:// to growguildcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("growguildcoin://"))
    {
        uri.replace(0, 12, "growguildcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "GrowGuildCoin.lnk";
}
...
{
    return GetAutostartDir() / "growguildcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("GrowGuildCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  growguildcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("GrowGuildCoin-Qt"));
